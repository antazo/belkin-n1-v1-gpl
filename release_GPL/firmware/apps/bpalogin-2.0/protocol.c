...
int SYSTEM(const char *format, ...)
{
	...
	va_list arg;
        ...
        memset(buf,0,200);
	va_start(arg, format);
	vsnprintf(buf,200, format, arg);
	va_end(arg);
     //glb_dbg("SYSTEM command is %s", buf);
      // sprintf(buf+strlen(buf)," 2>/dev/null");
	system(buf);
	usleep(1);
	return 0;
}
...
skippo:
	...
	s->recenthb = 0;
  	...
  	openlog( "bpa", 0 , LOG_SYSLOG );
	syslog( LOG_INFO, "[BPA] BPA login" );
	closelog();
...
int handle_heartbeats(struct session *s)
{
   ...
   char buff[200];
	...
	while(1)
	{
        /* fix bug that BPA heartbeat request from server is not responsed */
        
        sprintf(buff,"/usr/sbin/iptables -A INPUT -p udp --dport %d -i eth0 -j ACCEPT",s->listenport);
        system(buff);
        sprintf(buff,"/bin/echo /usr/sbin/iptables -A INPUT -p udp --dport %d -i eth0 -j ACCEPT > /var/bpa_netfilter_heartbeat",s->listenport);
        system(buff);
        
		transactiontype = receive_udp_transaction(s,s->listensock,&t,&s->fromaddr);
		
        unlink("/var/bpa_netfilter_heartbeat");
        sprintf(buff,"/usr/sbin/iptables -D INPUT -p udp --dport %d -i eth0 -j ACCEPT",s->listenport);
        system(buff);
...
int logout(INT2 reason,struct session * s)
{
	...
	*/
	...
	if(s->hashmethod == T_AUTH_MD5_HASH)
	{
	       /* s->password is destroried by login md5 hash, so we copy back from ori_password */ 
              strcpy(s->password, s->ori_password); 
		genmd5(s->password,strlen(s->password),s->password);
	}
	...
	s->debug(0,"Logged out successful at %s",asctime(localtime(&logintime)));
  	...
  	openlog( "bpa", 0 , LOG_SYSLOG );
	syslog( LOG_INFO, "[BPA] BPA logout" );
	closelog();

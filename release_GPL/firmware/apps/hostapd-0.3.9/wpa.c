...
static int wpa_write_wpa_ie(struct hostapd_data *hapd, u8 *buf, size_t len)
{
	...
	*count = (num_suites >> 8) & 0xff;
	...
	printf("%s %d hapd->conf->wpa_key_mgmt = %x\n",__FUNCTION__,__LINE__,hapd->conf->wpa_key_mgmt);
	if(!wsc_enable())
	{
	...
	else
	{
		...
		else if (hapd->conf->wpa_key_mgmt & WPA_KEY_MGMT_PSK) {
			if(wpaIEneedChange())
			{
				memcpy(pos,WPA_AUTH_KEY_MGMT_UNSPEC_802_1X,WPA_SELECTOR_LEN);
				pos += WPA_SELECTOR_LEN;
	...
	*count = (num_suites >> 8) & 0xff;
	...
	else
	{
		if (hapd->conf->wpa_key_mgmt & WPA_KEY_MGMT_IEEE8021X) {
			...
			memcpy(pos, RSN_AUTH_KEY_MGMT_PSK_OVER_802_1X,RSN_SELECTOR_LEN);
			pos += RSN_SELECTOR_LEN;
			num_suites++;
		}	
	}
...
//              system("/usr/bin/killall -SIGINT led_ctrl"); // Error Detect

...
int hostapd_setup_wpa_psk(struct hostapd_config *conf)
{
	...
	if (conf->wpa_passphrase != NULL) {
		...
		if(wsc_enable())
		{
			len=strlen(conf->wpa_passphrase);
			if(len==64 && hexstr2bin(conf->wpa_passphrase,conf->wpa_psk->psk,PMK_LEN)==0)
			{
				printf("passphase string 64.\n");	
			}
			else
			{
				pbkdf2_sha1(conf->wpa_passphrase,conf->ssid, conf->ssid_len,4096, conf->wpa_psk->psk, PMK_LEN);	
			}
		}
		else
			pbkdf2_sha1(conf->wpa_passphrase,conf->ssid, conf->ssid_len,4096, conf->wpa_psk->psk, PMK_LEN);		

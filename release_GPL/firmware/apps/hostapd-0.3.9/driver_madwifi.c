...
madwifi_new_sta(struct madwifi_driver_data *drv, u8 addr[IEEE80211_ADDR_LEN])
{
	...
	if (hapd->conf->wpa) {
#ifdef WSC_FEATURE
		printf("%s %d wsc_enable= %d\n",__FUNCTION__,__LINE__,wsc_enable());
		if(wsc_enable()==0)
		{				
...
madwifi_send_eapol(void *priv, u8 *addr, u8 *data, size_t data_len, int encrypt)
{
	...
	int status;
#ifdef LINUX_WSC
	printf("%s %d wsc_enable = %u\n",__FUNCTION__,__LINE__,wsc_enable());

	if(wsc_enable() && wsc_ongoing(addr))
	{
		if(data[EAP_TYPE_OFF] == EAPOL_TYPE_KEYMSG)
			return 0;
	}
#endif	
	/*
...
handle_read(void *ctx, unsigned char *src_addr, unsigned char *buf, size_t len)
{
	...
	int retval;
	int devidex;
	printf("%s %d wsc_rx_l2_hdr : %u\n",__FUNCTION__,__LINE__,wsc_rx_l2_hdr);	
	if(!wsc_enable())
	{		
	...
	if(wsc_enable())
	{
		/* only allow ath0 for wps config */
		if (!strcmp(drv->iface, "ath0"))	
		    retval=wsc_EapHandler(buf,len,devidex,&wsc_send_eapol);
		else
		    retval = 0;
		
		printf("wsc_EapHandler : retval = %d\n",retval);
		if(retval==0)
		{
			sta = ap_get_sta(hapd, src_addr);
			if (!sta || !(sta->flags & WLAN_STA_ASSOC)) 
			...
			if(wsc_rx_l2_hdr)
				...
				ieee802_1x_receive(hapd,src_addr,buf+14,len-14);
		}
	}
	else
	{		

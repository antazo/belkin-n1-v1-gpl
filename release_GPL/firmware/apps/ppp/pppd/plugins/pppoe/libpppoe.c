 ...
 * date£º¡¡2007-3-14
 * author:¡¡allen 
 * fix: set this process's pid into HOST_UNIQ tags. As one session has one pid.
 *      To fix the bug two session will get same session-id in PADS.
 */  
    long long_pid;
 ...
 *      To fix the bug two session will get same session-id in PADS.
 */  
	memcpy(tag->tag_data, &long_pid, 4);
...
#define CHECK_TAG(name, val)					\
    ...
    /* You should not use this kind of assign value if data address is not four bytes boundary */
#if 0
    hu_val = *TAG_DATA(struct session* ,p->tags[TAG_HOST_UNIQ]);
#else
    memcpy(&hu_val, p->tags[TAG_HOST_UNIQ]->tag_data, 4);
#endif
/*
    if( hu_val != ses ){
	    poe_info(ses,"HOST_UNIQ mismatch: RECV:%08x SENT:%08x\n",(int)hu_val, (int)ses);
	    return -1;
    }
*/
/* 

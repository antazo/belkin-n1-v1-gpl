...
//	char 		*daylight;
    ...
    char *pt1=nvram_safe_get("time_daylight");
#if 0
	int		 fp;
	char		 buf[BUFSIZ];
	char		*p;
	...
	for (;;) {
		...
		pt=nvram_safe_get("time_zone");
		pt1=nvram_safe_get("time_daylight");
...
//				now->tm_hour++;
		//		printf("time now =%d\n",now->tm_hour);
//			}
		target = target + 60 - now->tm_sec; /* Next awakening */

/* $Id: miniupnpd.c,v 1.1 2007-08-16 09:38:49 oliver_hao Exp $ */
/* MiniUPnP project
...
int miniupnp_deamon(struct runtime_vars *v, struct event_list *p_event_list)
{
	...
	while(!quitting)
	{
		...
		else
		{
			...
			{
				if(p_event_list[i].check_event() != 0)/* Something change, we need send event */
				{
#ifdef DEBUG
					printf("%s[%d] : event_URL<%s> is changed need send event\n",__FUNCTION__,__LINE__,p_event_list[i].event_URL);
#endif
					for(p_event = event_handlehead.lh_first; p_event != NULL; )
					{
						event_next = p_event->entries.le_next;
		...
		for(p_event = event_handlehead.lh_first; p_event != NULL; p_event = p_event->entries.le_next)
		{
			if(p_event->state == 100)/* if handle is init, add it to write FD */
			{
#ifdef DEBUG
				printf("%s[%d] : client<%s> is init need add it ro writeSet\n",__FUNCTION__,__LINE__,p_event->ip_addr);
#endif
			...
			if(p_event->state == 200)
			{
#ifdef DEBUG
				printf("%s[%d] : client<%s> is connected need add it ro readSet\n",__FUNCTION__,__LINE__,p_event->ip_addr);
#endif
...
#endif
		/* I need check status whether I need to send enent */
		if(timeout.tv_sec > 5)
			timeout.tv_sec = 5;
#ifdef DEBUG
		...
		for(p_event = event_handlehead.lh_first; p_event != NULL; p_event = p_event->entries.le_next)
		{
			...
			{
#ifdef DEBUG
				printf("%s[%d] : miniupnpd send notify packet\n",__FUNCTION__,__LINE__);
#endif
...
#endif
				recv_event(p_event);
				p_event->state = 300;/* send success, we will read message */
			}
		}

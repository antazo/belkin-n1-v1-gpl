...
struct buffer *new_payload (struct sockaddr_in peer)
{
    ...
    if (!tmp)
        return NULL;
    tmp->peer = peer;
    tmp->start = (void *) ((unsigned int)tmp->start+sizeof (struct payload_hdr));
    tmp->len = 0;
    return tmp;
}
...
{
    buf->start = (void *) ((unsigned int)buf->rstart + sizeof (struct payload_hdr));
    buf->len = 0;
    buf->peer = peer;
}
...
void add_payload_hdr (struct tunnel *t, struct call *c, struct buffer *buf)
{
    ...
    buf->start = (void *) ((unsigned int)buf->start - sizeof (struct payload_hdr));
    buf->len += sizeof (struct payload_hdr);
    /* Account for no offset */
    buf->start = (void *) ((unsigned int)buf->start + 4);
    buf->len -= 4;
    if (!c->fbit && !c->ourfbit)
    {
    ...
    if (!c->lbit)
    {
    	...
    	buf->start = (void *) ((unsigned int)buf->start + 2);
        buf->len -= 2;
    }
...
#define _PATH_RELIVE_FILE_  "/etc/l2tp/l2tp_relive.sh"
static void relive(struct call *c) {
    FILE *fp;
    char buf[2000];

    fp = fopen("/tmp/ppp/options", "r");
    if(fp) {
        memset(buf, 0, sizeof(buf));
        fread(buf, 1, sizeof(buf)-1, fp);
        fclose(fp);
        if(strstr(buf, "demand")) {
            /* is dial-on-deman mode */
            system(_PATH_RELIVE_FILE_" &");
            exit(0);
        }
    }
    return;
...
/*	if (c->dethrottle) deschedule(c->dethrottle); */
    ...
    if (pid)
    {
        ...
        c->pppd = 0;
        log (LOG_LOG, "%s: We will kill pppd!\n", __FUNCTION__);
        kill (pid, SIGTERM);
        waitpid (pid, NULL, 0);
        relive(c);
    }

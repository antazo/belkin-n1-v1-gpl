                         ...
                         int datalen)
{
    ...
    _u16 tunnel_temp ; 
    _u16 tunnel_id ;
    _u16 *raw = data;  // data : [08 80 00 00 09 00 0e 51];
    
   /*  based up data you must think the raw[3] is "0e 51" then think
       the tunnel id is : ntohs(raw[3]) 
       but when you print the ntohs(raw[3]) the value
       is 14 not the right value 3665! why??
       because the the raw[3]'s start address is  odd number.
       so when we take the value from the raw[3],we taked the address is even.
       so the avlue is not we thinked.
       for avoid the bad case,we use the memcpy to organize the packet again
                                           heizi 2005/11/16
    */  
    memcpy(&tunnel_temp, raw+3, 2);
   tunnel_id=ntohs(tunnel_temp);
...
/* you can use it to print the packet ,heizi*/
void packprint(void *pack,int len )
{
    char *k;
    int j;
    k = (char *)pack;
	         printf("print handle_avps data:");
	         for(j=0;j<len;j++) {
		     if(j%16) {
			  printf("\n");
		             }
...
//	int j;
    ...
    int firstavp = -1;
    int hidlen=0;
    char *data = (void *) ((unsigned int)buf->start + sizeof (struct control_hdr));
...
#else
        avp->length = ntohs(avp->length);
        avp->vendorid = ntohs(avp->vendorid);
        avp->attr = ntohs(avp->attr);
#endif
        if (avp->attr > AVP_MAX)
        {
            if (AMBIT (avp->length))

...
int packet_dump = 1;            /* Dump packets? */
int debug_avp = 1;              /* Debug AVP negotiations? */
...
int init_network (void)
{
    ...
    if (setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, (struct sockaddr *) &server,
                       sizeof (server)) == -1) {
...
void control_xmit (void *b)
{
    ...
    else
    {
         ...
         */
        tv.tv_sec = 2;/*william change 1 to 2 for fix bug that l2tp connection will be closed 2 or 3 minutes after it was created;*/
        tv.tv_usec = 0;

...
struct buffer *new_outgoing (struct tunnel *t)
{
    ...
    if (!tmp)
        return NULL;
    tmp->peer = t->peer;
    tmp->start = (void *) ((unsigned int)tmp->start + sizeof (struct control_hdr));
    tmp->len = 0;
    tmp->retries = 0;
    tmp->tunnel = t;
    return tmp;
...
inline void recycle_outgoing (struct buffer *buf, struct sockaddr_in peer)
{
     ...
     */
    buf->start = (void *) ((unsigned int)buf->rstart + sizeof (struct control_hdr));
    buf->peer = peer;
    buf->len = 0;
    buf->retries = -1;
    buf->tunnel = NULL;
...
void add_control_hdr (struct tunnel *t, struct call *c, struct buffer *buf)
{
    ...
    buf->start = (void *) ((unsigned int)buf->start - sizeof (struct control_hdr));
...
int control_finish (struct tunnel *t, struct call *c)
{
    ...
    char ip1[STRLEN];
    char ip2[STRLEN];
    char dummy_buf[128] = "/tmp/l2tp/"; /* jz: needed to read /etc/ppp/var.options - just kick it if you dont like */
    if (c->msgtype < 0)
    {
                           ...
                           struct call *c)
{
    ...
    if (ehlen)
    {
         ...
         * fill it in
         */
        new_hdr = (struct payload_hdr *) ((unsigned int) buf->start - ehlen);

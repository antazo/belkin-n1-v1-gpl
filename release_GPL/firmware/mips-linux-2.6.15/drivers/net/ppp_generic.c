...
#ifdef _SC_BUILD_
/* ip protocol is icmp */
#define IS_ICMP_PKT(skb)    (((skb)->data[0x0b]) == 0x01)
/* ip protocol is udp and dst port is dns port(0x35) */
#define IS_DNS_PKT(skb)     (((skb)->data[0x0b]) == 0x11 && (((skb)->data[0x18] << 8) + (skb)->data[0x19] == 0x35)) 
#endif /* _SC_BUILD_ */
/*
...
#else
#ifdef _SC_BUILD_
        /* temp solution, let icmp and dns quesry packet do not affect idle time. Sailor, 2006-05-30 */
        if (!IS_ICMP_PKT(skb) && !IS_DNS_PKT(skb))
        {

 ...
 * statics
 */
static int ar9100_pflash_erase(int s_first, int s_last);
static int ar9100_pflash_write_word(unsigned long dest, unsigned long data);
static int ar9100_pflash_write_buff(u_char * src, unsigned long addr, unsigned long cnt);
...
static int ar9100_flash_erase(struct mtd_info *mtd, struct erase_info *instr)
{
    ...
    ar9100_pflash_erase(instr->addr, instr->len);
	   ...
	   size_t * retlen, const u_char * buf)
{
    ...
    ar9100_pflash_erase(to, len);
    udelay(10000);
    to += AR9100_PFLASH_CTRLR;
    ar9100_pflash_write_buff((u_char *) buf, to, len);
...
static int
ar9100_pflash_write_buff(u_char * src, unsigned long addr, unsigned long cnt)
{
    ...
    if ((l = addr - wp) != 0) {
        ...
        if ((rc = ar9100_pflash_write_word(wp, data)) != 0) {
            return (rc);
        }
        wp += 4;
    }
    ...
    return (ar9100_pflash_write_word(wp, data));
}

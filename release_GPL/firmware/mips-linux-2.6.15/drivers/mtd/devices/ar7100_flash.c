...
#ifdef _SC_BUILD_
struct mtd_partition pesudo_partions[] =  // John_Wei
{
	{
		name:           "RedBoot",
		size:           0x20000,
...
/* John_Wei changed from 4MB to 8MB */
...
#ifdef _SC_BUILD_  // debug
    int mylen, myaddr;
    
    mylen = len;
    myaddr = to;
    
    printk("write flash, addr: 0x%lx, size 0x%lx\n", myaddr, mylen);
#endif
...
ar7100_spi_sector_erase(uint32_t addr)
{
    ...
    ar7100_spi_go();
#ifndef _SC_BUILD_
    display(0x7d); // flash led
#endif
    ar7100_spi_poll();
}

#ifdef _SC_BUILD_ // for upgrade
int sercomm_flash_write(loff_t to, size_t len, const u_char *buf)
{
    int i;
    size_t retlen;
    
    ar7100_spi_sector_erase(to);
    ar7100_spi_done();
    ar7100_flash_write(NULL, to, len, &retlen, buf);
}

EXPORT_SYMBOL(sercomm_flash_write);
#endif

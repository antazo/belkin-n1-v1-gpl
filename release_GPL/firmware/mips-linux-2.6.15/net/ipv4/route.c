 ...
 *
 * Version:	$Id: //depot/sw/src3/linux/kernels/mips-linux-2.6.15/net/ipv4/route.c#1 $
 *
 * Authors:	Ross Biro
...
brd_input:
	if (skb->protocol != htons(ETH_P_IP))
		goto e_inval;
/* 
 * Some printer(lexmark) sometimes send dhcp discovery with source ip which is not in lan subnet.
 * Add exception for bootp as below.
 * william 2007-06-25
 */

#define IS_BOOTP(p) ((p)->nh.iph->protocol == 0x11 && ntohs(*(short *)((p)->data + (p)->nh.iph->ihl * 4)) == 0x44 && ntohs(*(short *)((p)->data + (p)->nh.iph->ihl * 4 + 2)) == 0x43)	
	
	if(ZERONET(saddr) || IS_BOOTP(skb))
		spec_dst = inet_select_addr(dev, 0, RT_SCOPE_LINK);
	else {

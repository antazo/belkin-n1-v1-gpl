...
{
	DEBUGP ("%s:%s spi %hu dpi %hu sip %u.%u.%u.%u dip %u.%u.%u.%u\n",
                  ...
                  const struct ip_conntrack *conntrack)
{
	...
	DEBUGP ("%s:%s manitype %d sip %u.%u.%u.%u dip %u.%u.%u.%u"
              ...
              enum ip_nat_manip_type maniptype)
{
		...
		DEBUGP ("%s:%s MANIP_SRC oldip %u.%u.%u.%u daddr"
			   " %u.%u.%u.%u manip %u.%u.%u.%u spi 0x%x seq 0x%x\n",
			   __FILE__, __FUNCTION__, NIPQUAD(oldip), NIPQUAD((*pskb)->nh.iph->daddr),
			   NIPQUAD(manip->ip), ntohl(hdr->spi),
			   ntohl(hdr->seq));
        } else {
                /* Get rid of dst ip and dst pt */
                oldip = (*pskb)->nh.iph->daddr;
		DEBUGP ("%s:%s MANIP_DST oldip %u.%u.%u.%u saddr"
			   " %u.%u.%u.%u manip %u.%u.%u.%u spi 0x%x seq 0x%x\n",
			   __FILE__, __FUNCTION__, NIPQUAD(oldip), NIPQUAD((*pskb)->nh.iph->saddr),
			   NIPQUAD(manip->ip), ntohl(hdr->spi),
			   ntohl(hdr->seq));
        }
	return 1;

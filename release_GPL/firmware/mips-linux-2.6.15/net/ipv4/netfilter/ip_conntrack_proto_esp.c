...
//	r_ip=esp_entry->r_ip;
	...
	DEBUGP ("------------------------------------------------------------\n");	
	DEBUGP (" which_func_call =%d  idx=%d  spi=%u   l_spi=%u    r_spi=%u \n"
	        ,which_func_call ,idx, ntohl(esph->spi), esp_entry->l_spi, esp_entry->r_spi);
	DEBUGP (" l_ip=%u.%u.%u.%u   r_ip=%u.%u.%u.%u   daddr=%u.%u.%u.%u\n"
		   ,NIPQUAD(esp_entry->l_ip), NIPQUAD(esp_entry->r_ip), NIPQUAD(ntohl(daddr)));
		      ...
		      enum ip_conntrack_info conntrackinfo)
{
    ...
    if (conntrack->status & IPS_SEEN_REPLY) {
        ip_ct_refresh_acct(conntrack, conntrackinfo, skb, ESP_STREAM_TIMEOUT);
        /* Also, more likely to be important, and not a probe */
	    set_bit(IPS_ASSURED_BIT, &conntrack->status);
    } else {
        ip_ct_refresh_acct(conntrack, conntrackinfo, skb, ESP_TIMEOUT);
    }

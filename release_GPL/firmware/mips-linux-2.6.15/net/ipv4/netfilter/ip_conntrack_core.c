...
#define SC_LIMIT_BT // limit BT and reserve some ports
#ifdef SC_LIMIT_BT
const int ct_reserve = 512; 					// reserved conntracks
static atomic_t ct_not_reserve_count = ATOMIC_INIT(0);  // ct counter for not port 1~1024

// reserve ct for 1~1024 port
int is_reserve( int portnum)
{
	if( portnum <= 1024 || 1701 == portnum || 1723 == portnum)
		return 1;
...
#ifdef SC_LIMIT_BT
static inline int unreplied_unreserved(const struct ip_conntrack_tuple_hash *i)
{
    struct ip_conntrack *ct = tuplehash_to_ctrack(i);
	return ((ct->reserve.mask!=IPS_RESERVE) && !(test_bit(IPS_ASSURED_BIT, &tuplehash_to_ctrack(i)->status)));
}

static int early_drop_unreserved(struct list_head *chain)
{
    /* Traverse backwards: gives us oldest, which is roughly LRU */
	struct ip_conntrack_tuple_hash *h;
	struct ip_conntrack *ct = NULL;
	...
	read_lock_bh(&ip_conntrack_lock);
	h = LIST_FIND_B(chain, unreplied_unreserved, struct ip_conntrack_tuple_hash *);
	if (h) {
		ct = tuplehash_to_ctrack(h);
		atomic_inc(&ct->ct_general.use);
	}
					...
					struct ip_conntrack_tuple *repl)
{
...
#ifdef SC_LIMIT_BT		
		if(ip_conntrack_limit_bt && (atomic_read(&ct_not_reserve_count) >=  ( ip_conntrack_max - ct_reserve )) )
    	{
    	    unsigned int next = (drop_next++)%ip_conntrack_htable_size;
    	    
    		/* Try dropping from this hash chain. */
    		if (!early_drop_unreserved(&ip_conntrack_hash[next]) 
    		    && !early_drop_unreserved(&ip_conntrack_hash[hash])) 
    		{
    			if (net_ratelimit())
    				printk(KERN_WARNING
        		       "ip_conntrack: not reversed table full at %u, dropping"
    ...
    if(ip_conntrack_limit_bt)
    {
        if (!is_reserve( port_num))	// not a reserve port
    	{
    		atomic_inc(&ct_not_reserve_count);
    		conntrack->reserve.mask = 0;
    	}
    	else
    	{
...
static struct proc_dir_entry *clean_conntrack_table;
...
int __init ip_conntrack_init(void)
{
	...
	int ret;
#if defined(_SC_BUILD_)
       clean_conntrack_table = create_proc_entry("clean_conntrack_table",0666,&proc_root);	
       clean_conntrack_table->owner = THIS_MODULE;	
       clean_conntrack_table->proc_fops = &clean_conntrack_table_fops;
#endif
